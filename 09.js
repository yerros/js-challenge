...
function OrderFebruari(a){
    var orderfebruari = []
    for(let i = 0; i < a.length; i++){
        var februari = a[i].created_at.slice(5,7)
        if(februari == 02){
            orderfebruari.push(a[i])
        }
    }
    console.log(orderfebruari)
}

function OrderBy(a,customer){
    orderTotal = 0;
    orderArr = []
    for(let i = 0; i < a.length; i++){
        if(a[i].customer.name == customer){
            orderArr.push({ items: a[i].items })   
            console.log(a[i].length)
            for(let b=0; b < a[i].length; b++){
                console.log(a[i].items[b])
            }             
        }
    }
...
OrderFebruari(orderList)
//OrderBy(orderList,'Ari')